...
start_sql_proxy() {
    ...
    # 3. Start Proxy in Background (Port 5432) with Private IP
...
configure_environment() {
    ...
    if [ ! -d "$ENV_DIR" ]; then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            info "Creating GCS bucket '$BUCKET_NAME' for Terraform state..."; gsutil mb -p "$GCP_PROJECT_ID" -l europe-north1 "gs://${BUCKET_NAME}" || warn "Bucket 'gs://${BUCKET_NAME}' may already exist. Continuing..."
        fi
