...
trigger_builds() {
    ...
    info "Triggering backend build..."; gcloud builds triggers run "${BE_SERVICE_NAME}-trigger" --region="us-east4" --branch="$BRANCH_TO_USE" --project="$GCP_PROJECT_ID" || true
    info "Triggering frontend build..."; gcloud builds triggers run "${GCP_PROJECT_ID}-trigger" --region="us-east4" --branch="$BRANCH_TO_USE" --project="$GCP_PROJECT_ID" || true
